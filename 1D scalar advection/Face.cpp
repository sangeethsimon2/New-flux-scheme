...
void Face::calculateNormal(){
	...
	// For each defining point, we retrieve its x, y and z value using method in Point.cpp class.
	for(int i=0;i<3;i++){
		...
		//if(this->id == 0)
		//{
		//	cout<< definingPoints[i]->getId()<<"\n";
		//}
	}

	

	// Obtain 3 components of the vector joining point 0 to points 1 and 2.
	a[0] = x[1]-x[0];	
	a[1] = y[1]-y[0];	
	a[2] = z[1]-z[0];	
	...
	// Vector cross product between these vectors to obtain the 3 cartesian components of the normal vector 
	Normal[0] = a[1]*b[2]-a[2]*b[1];
	Normal[1] = a[2]*b[0]-a[0]*b[2];
	Normal[2] = a[0]*b[1]-a[1]*b[0];
	
	//Obtain the magnitude of the normal vector
...
void Face::orderDefiningPoints(){
	...
	if (this->facetype == FaceType::Tri){
		...
		*/
		...
		// Retrieve the x,y,z locations of the four points making up the face
		...
		// If points are read from OpenFoam file, then this block does not execute
		...
		// If points are read from OpenFoam file, then this block  executes
